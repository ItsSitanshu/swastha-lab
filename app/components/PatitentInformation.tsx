...
const ExternalReferenceTemplate: React.FC<ExternalReferenceProps> = ({
  onSelectReference,
}) => {
  const referenceTypes = ["X-Ray", "CT Scan", "ECG", "MRI"];
  ...
  return (
    <div className="col-span-3 border-b border-background-20">
      {referenceTypes.map((type) => (
        <button
          key={type}
          onClick={() => onSelectReference(type)}
          className="p-4 mt-4 w-full rounded-lg bg-mod text-light border border-background-20 transition duration-200 hover:bg-mod-dark"
        >
          {type}
        </button>
      ))}
...
const PatientInformation: React.FC = () => {
  ...
  const handleAddMedication = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("medications")
        .insert([{ name: newMedName, mrp: newMedMrp, desc: newMedDesc }]);
  ...
  const handleAddReference = () => {
    alert(
      `Reference Type: ${selectedReferenceType}, Observation: ${selectedObservation}`
    );
  ...
  return (
    <div className="p-8 grid grid-cols-12 gap-8">
      ...
      <div className="col-span-3 h-full flex flex-col gap-6 border-b border-background-20">
        <textarea
          className="p-4 rounded-lg bg-background-80 text-foreground-100 border border-foreground-20 focus:outline-none focus:border-mod"
        ...
        {loading ? (
          ...
          <ul className="bg-background-60 rounded-lg border border-background-20">
            {medications.slice(0, 3).map((med: any, i: number) => (
              <li
                key={med.id}
                className={`p-3 border-b border-x border-foreground-20 cursor-pointer ${
                  i === 2 && "rounded-b-xl"
                }`}
      ...
      {showReferenceModal && selectedReferenceType && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div className="bg-background-60 p-6 rounded-lg w-96">
            <h3 className="text-foreground-100 font-semibold">
              Add {selectedReferenceType} Observation
            </h3>
            ...
            <div className="mt-4">
              ...
              >
                ...
                {observations[selectedReferenceType]?.map(
                  (obs: any, index: any) => (
            ...
            <div className="flex justify-between mt-4">
              <button
                ...
                className="w-1/2 p-3 bg-mod text-light rounded-lg transition duration-200 hover:bg-mod-dark"
              >
                Save Reference
              </button>
              <button
                onClick={() => setShowReferenceModal(false)}
                className="w-1/2 p-3 bg-gray-500 text-light rounded-lg transition duration-200 hover:bg-gray-600"
              >
                Cancel
              </button>
            </div>
      ...
      <div className="col-span-3 bg-background-50 p-6 rounded-lg border border-background-20 mt-8">
        ...
        <ul>
          {selectedMedications.map((med: any) => (
            <li
              key={med.id}
              className="p-2 border-b border-background-20 flex justify-between"
            >
              <p className="text-foreground-100 font-semibold">{med.name}</p>
              <button
                onClick={() => handleRemoveMedication(med.id)}
                className="text-red-500 cursor-pointer transition duration-200 hover:text-red-700"
              >
                Remove
              </button>
            </li>
